...
function ItemLine(props) {
  ...
  const client = props.stores.Clients.items.find(
  ...
  return (
    ...
    >
      ...
      >
        <span className={'mr-2 align-self-center text-secondary'}>
          {props.lineNumber}
        </span>
        ...
        <span className={Style.jira + ' text-center'}>
          {jiraUrl && props.track.ticket && (
            <a
              href={jiraUrl + '/browse/' + props.track.ticket}
              target={'_blank'}
            >
        ...
        {!props.track.active && (
          <Button onClick={props.start} className="ml-2">
            Start
          </Button>
        )}

        {props.track.active && (
          <Button onClick={props.stop} className="ml-2" variant="danger">
            Stop
          </Button>
        )}

        <Button variant="secondary" className="ml-2" onClick={props.edit}>
          Edit
        </Button>

        <Button variant="secondary" className="ml-2" onClick={props.delete}>
          Delete
        </Button>
      </Row>
    </OverlayTrigger>
