...
class chromeStore extends StoreClass {
  ...
  @action getFieldFromJira(field) {
    ...
    if (field == 'ticket') {
      ...
      return chrome.runJS(
        this.currentTab,
        'document.querySelector(".property-list .issue-link").textContent'
      );
    } else if (field == 'epic') {
      return chrome
        .runJS(
          this.currentTab,
          "document.querySelector(\"[data-fieldtype='gh-epic-link'] a\").href.split('/').pop()"
        )
        .then(result => {
          if (result) {
          ...
          return this.getEpicLinkId().then(epicLinkId => {
            return this.getEpicLink(
              epicLinkId,
              this.getFieldFromJira('ticket')
            );
          });
        })
        .catch(e => {
  ...
  @action getFieldFromUtz(field) {
    ...
    if (field == 'date') {
      return chrome.runJS(
        this.currentTab,
        'document.querySelector("#contentwrapper .heading").innerText.match(/\\d\\d.\\d\\d.\\d\\d\\d\\d/)'
      );
    }
  }
  ...
  getEpicLinkId() {
    return fetch(
      this.rootStore.Settings.getSetting('jiraUrl') + `/rest/api/2/field`,
      {
        method: 'GET',
        headers: {
  ...
  getEpicLink(epicLinkId, ticket) {
    return fetch(
      this.rootStore.Settings.getSetting('jiraUrl') +
        `/rest/api/latest/issue/${ticket}?fields=issuetype,parent,${epicLinkId}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json;charset=UTF-8'
  ...
  @action saveJira(track) {
    ...
    function injectTimeLogField(query) {
      return instance.injectField(
        "document.getElementById('log-work-time-logged')",
        query
      );
    }
    ...
    chrome.runJS(
      this.currentTab,
      "document.getElementById('log-work').click()"
    );
    ...
    injectTimeLogField(
      "document.getElementById('log-work-time-logged').value = '" +
        elapsedTime.hours +
        'h ' +
        elapsedTime.minutes +
        "m'"
    ...
    injectTimeLogField(
      "document.getElementById('log-work-date-logged-date-picker').value = '" +
        time +
        "'"
    );
    ...
    setTimeout(function() {
      injectTimeLogField(
        "document.querySelector('iframe').contentWindow.document.querySelector('.mce-content-body').innerHTML = '<p>" +
          track.comment +
          "</p>';" +
          "document.querySelector('rich-editor').innerHTML = '<p>" +
          track.comment +
          "</p>'; " +
          "document.querySelector('#log-work #comment').value = '" +
          track.comment +
          "'; "
      );
    }, 1000);
  ...
  injectField(checkedField, query) {
    const instance = this;
    return chrome.runJS(this.currentTab, checkedField).then(result => {
      if (result) {
  ...
  @action saveUTZ(track) {
    const elapsedTime = dateTime.timeDiffSplitted(
      track.startTime,
      track.active ? new Date() : 0,
      track.elapsedTime
    );

    //open popup
    chrome
      .runJS(this.currentTab, "document.getElementsByTagName('iframe')[0]")
      .then(result => {
        if (!result) {
          chrome.runJS(
            this.currentTab,
            "document.querySelector('#addRowLink').click()"
          );
        }
      });
